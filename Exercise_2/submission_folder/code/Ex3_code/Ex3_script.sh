...
do
    ...
    do
        ...
        srun $cs $ts $ss --nodes=$Nd --ntasks-per-node=$TPN ./Ex3 -c $mc -p 2 -b 100 -h $Nd -g $txtgen
        srun $cs $ts $ss --nodes=$Nd --ntasks-per-node=$TPN ./Ex3 -c $mc -p 10 -b 100 -h $Nd -g $txtgen
