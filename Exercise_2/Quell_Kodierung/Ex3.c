...
int MY_Allreduce_P(tuwtype_t *sendbuf, tuwtype_t *recvbuf, int count, int size, int rank)
{
    ...
    int masterrank;
    int blockid;
    int numblocks = floor(count/blockSize); // number of standardsized blocks
    int groupsize = size < numblocks ? size : numblocks; // standardgroupsize
    int blockgroup = 0;

    while(blockgroup < floor(numblocks/groupsize)) { // for standardsized blockgroups
        blockid = blockgroup*groupsize + (rank+size-1)%groupsize;
        for(int blockcounter = 0; blockcounter < groupsize; blockcounter++)
        {
            masterrank = blockid % size;

            if (rank == masterrank) 
            {
                MPI_Recv(tmpbuf, blockSize, MPI_DOUBLE, (rank+1)%size, 2*blockid, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
                for (int j = 0; j < blockSize; j++)
                {
                    recvbuf[blockid*blockSize+j] = tmpbuf[j] > sendbuf[blockid*blockSize+j] ? tmpbuf[j] : sendbuf[blockid*blockSize+j];
                }
            }
            else if (rank == (masterrank+size-1) % size)
            {
                MPI_Send(sendbuf + blockid*blockSize, blockSize, MPI_DOUBLE, (rank+size-1)%size, 2*blockid, MPI_COMM_WORLD);
            }
            ...
            else
            {
                ...
                MPI_Send(recvbuf + blockid*blockSize, blockSize, MPI_DOUBLE, (rank+size-1)%size, 2*blockid, MPI_COMM_WORLD);
            }

            blockid = blockgroup*groupsize + (blockid+1)%groupsize;
        }

        blockgroup++;
    }
    // blockgroup of not standardsize
    if (numblocks%groupsize != 0)
    {
        blockid = blockgroup*groupsize + (rank+size-1)%groupsize;
        for(int blockcounter = 0; blockcounter < numblocks%groupsize; blockcounter++)
        {
            masterrank = blockid % size;
        ...
        if (count % blockSize != 0)
        {
            ...
            if (rank == masterrank) 
            {
                MPI_Recv(tmpbuf_trunc, count%blockSize, MPI_DOUBLE, (rank+1)%size, 2*blockid, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
                for (int j = 0; j < count%blockSize; j++)
                {
                    recvbuf[blockid*blockSize+j] = tmpbuf_trunc[j] > sendbuf[blockid*blockSize+j] ? tmpbuf_trunc[j] : sendbuf[blockid*blockSize+j];
                }
            }
            else if (rank == (masterrank+size-1) % size)
            {
                MPI_Send(sendbuf + blockid*blockSize, count%blockSize, MPI_DOUBLE, (rank+size-1)%size, 2*blockid, MPI_COMM_WORLD);
            }
            ...
            else
            {
                ...
                MPI_Send(recvbuf + blockid*blockSize, count%blockSize, MPI_DOUBLE, (rank+size-1)%size, 2*blockid, MPI_COMM_WORLD);        
            }
        } 
    }
...
int main(int argc, char *argv[])
{
    ...
    for (int i = 0; i < count; i++) {
        ...
        printf("count-step: %d, block: %d, rank %d: %f, %f\n",i, (int)floor(i/blockSize), rank, recvbuf[i], testbuf[i]); // for debugging
    }
